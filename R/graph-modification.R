...
setRandomEdgeWeights <- function(
  graph,
  dist.fun= function(n) ceiling(runif(n,0,10)) ,
  overwrite=TRUE
){
  setAttr(graph, "edge", "weight",
          function(graph){ dist.fun(length(E(graph))) },
          overwrite
          )
}
...
setUniformEdgeWeights <- function(graph, overwrite=TRUE){
  setAttr(graph, "edge", "weight",
          function(graph){ rep(1, length(E(graph))) },
          overwrite
  )
}
...
setEmptyShortestPathPredecessors <- function(graph, overwrite=TRUE){
  setAttr(graph, "graph", "shortest_path_predecessors",
          function(graph){
            n <- length(V(graph))
...
setEmptyVertexFronts <- function(graph, overwrite=TRUE){
  setAttr(graph, "vertex", "front",
          function(graph){ rep(NA, length(V(graph))) },
          overwrite
  )
}
