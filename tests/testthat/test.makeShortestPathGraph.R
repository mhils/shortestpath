...
# test_that('makeEdgeWeights is a valid default factory', { edge.weights <-
# makeEdgeWeights(make_graph('Dodecahedron')) expect_equal(edge.weights, rep(1,30)) })
# test_that('makeVertexNames is a valid default factory', { vertex.names <-
# makeVertexNames(make_graph('bull')) expect_equal(vertex.names, c('A','B','C','D','E')) })
# test_that('makeVertexNames handles graphs with 27+ vertices', { vertex.names <-
# makeVertexNames(make_graph('Tutte')) expect_equal(vertex.names, 1:46) })
# test_that('makeVertexFronts is a valid default factory', { vertex.front <-
# makeVertexFronts(make_graph('Dodecahedron')) expect_equal(vertex.front, rep(NA,20)) })
# test_that('makeGraphMinDists is a valid default factory', { graph.min_dists <-
# makeGraphMinDists(make_graph('Tetrahedral')) expect_equal(graph.min_dists, matrix(c( 0, Inf, Inf,
# Inf, Inf, 0, Inf, Inf, Inf, Inf, 0, Inf, Inf, Inf, Inf, 0 ), ncol=4)) })
