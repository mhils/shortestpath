...
vignette: >
  %\VignetteIndexEntry{Shortest Path Algorithms}
...
All algorithms are textbook implementations, that is no special optimizations have been applied. In general, the performance is significantly worse than an optimized implementation, albeit the complexity is the same. For Bellman-Ford and Floyd-Warshall, we skip further steps if an iteration does not yield any improvements at all.

## Comparing A* Search and Dijkstra

One of the more interesting relations is the one between A* Search and Dijkstra, where the latter can be seen as a special case of the former one (with an empty heuristics function). To illustrate their behaviour, we try to find the shortest path on a grid with euclidean distances:
